...
class LogAnalyzerCLI {
  ...
  async watchFile(filePath: string, pollInterval: number = 1000): Promise<void> {
    try {
  ...
  async listWatchedFiles(): Promise<void> {
    try {
      const result = await this.client.request({
        method: 'tools/call',
        params: {
  ...
  async getRecentErrors(limit: number = 10): Promise<void> {
    try {
      const result = await this.client.request({
        method: 'tools/call',
        params: {
  ...
  private displayAnalysisResult(result: any): void {
    try {
      ...
      if (analysis.suggestedFixes.length > 0) {
        console.log('\n🛠️  SUGGESTED FIXES:');
        analysis.suggestedFixes.forEach((fix: string, index: number) => {
      ...
      if (analysis.relatedErrors.length > 0) {
        console.log('\n🔗 RELATED ERRORS:');
        analysis.relatedErrors.forEach((error: string, index: number) => {
      ...
      if (analysis.followUpQuestions.length > 0) {
        console.log('\n❓ FOLLOW-UP QUESTIONS:');
        analysis.followUpQuestions.forEach((question: string, index: number) => {
  ...
  private displayRecentErrors(errors: any[]): void {
